...
function placesSearchCB(data, status){
    if(status === daum.maps.services.Status.OK){
        ...
        return;
    }else if ( status === daum.maps.services.Stauts.ERROR){
...
function displayPlaces(data){
    ...
    let bounds = new daum.maps.LatLngBounds();  //검색한 부분으로 가는 함수
    ...
    for(let i = 0; i<data.length; i++){
        ...
        const el = document.createElement("div"); //js를 통해서 div태그 만들기
        const itemStr = `
        <div class="info">
